...
function update_cpu(widget)
	...
	local cpu_status = ((cpu0_status+cpu1_status)/2)
	fd:close()

	cpu0_status = string.format("%02d", cpu0_status)
	cpu1_status = string.format("%02d", cpu1_status)
	cpu_status = string.format("%04.1f", cpu_status)

	local percent = tostring(cpu_status) --string.match(status, "....")--"(%d?%d?%d.%d?%d?)%%")
	local cpu_icon = '\u{f126}'

	widget:set_markup("  <span color='#9370db'>" .. cpu_icon .. " 1: " .. cpu0_status .. "% | 2: " .. cpu1_status .. "% | CPU: " .. percent .. "% " .. "</span>")
end
